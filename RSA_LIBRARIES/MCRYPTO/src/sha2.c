...
---------------------------------------------------------------------------
 ...
 Changes [MJ tag]: Every unsigned long variable was changed to uint32_t for 
...
// MJ change unsigned long -> uint32_t
void sha256_hash(const unsigned char data[], uint32_t len, sha256_ctx ctx[1])
...
void sha512_hash(const unsigned char data[], uint32_t len, sha512_ctx ctx[1])
{   sha_32t    i = 0, cnt, j;
    sha_64t    *ptr, val;
