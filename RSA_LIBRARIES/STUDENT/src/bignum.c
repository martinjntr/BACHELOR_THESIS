...
---------------------------------------------------------------------------
 ...
 Changes [MJ tag]: -  Rewrite GCD function to non-recursive function
                   - miller-rabin add free() for memory release
---------------------------------------------------------------------------
*/
...
{/*
    ...
    if (B == 16) {
        ...
        for (i = a.size-1; i >= 0; i--)
            printf("%x",(var) a.tab[i]);
        printf("\n");
    } else {
        for (i = a.size-1; i >= 0; i--)
            printf("%5d: %u\n", i,(var) a.tab[i]);
        printf("B = %ld, %s\n", (uint64_t)B, a.sign==1?"positive":"negtive");
*/
}
...
bignum leftshift(bignum a, int k)
{
    ...
    /*
         MJ increase malloc size allocation, before can
         occurs overflows on line 

    */
    res.tab = (block *)malloc((res.size + 1) * sizeof(block));
...
bignum divi(bignum a, bignum n)
{
    ...
    // MJ removing variable tmp, unusable
...
int millerrabin(bignum n, int t)
{
    ...
    if (compare(n, three) == 0 || compare(n, two) == 0) {
        free(two.tab); // MJ Causing memory release
        free(three.tab); //MJ Causing memory release
        return 1;
    }
    ...
    while (t--) {
        ...
        while(!isone(beta) && compare(beta, n_minus_one) != 0) {
            ...
            while (j-- && compare(beta, n_minus_one)!=0) {
                ...
                free(bt);
                if (isone(beta)){
                    free(one.tab); // MJ Causing memory release
                    free(n_minus_one.tab); // MJ Causing memory release
                    free(r.tab); // MJ Causing memory release
                    free(beta.tab); // MJ Causing memory release
                    return 0;
                }
            }
...
bignum gcd(bignum a, bignum b)
{
    ...
    /*
        MJ comment recursion GCDfucntion 
        and replace it with non-recursive 
